...
# Папка для кэша (чтобы не тратить токены зря на одинаковые отзывы)
...
def build_prompt(raw_text: str) -> str:
    return f"""Ты — чистильщик отзывов для ресторана. 
Твоя единственная задача — привести любой кривой копипаст из Telegram, GastroReview или откуда угодно к ОДНОМУ чистому предсказуемому формату.

Правила (строго соблюдай):
- Каждый отзыв должен начинаться с новой строки.
- Формат строго такой: [DD.MM.YYYY HH:MM] Имя: текст отзыва
- Если дата неизвестна — пиши [Дата неизвестна]
- Если имя неизвестно — пиши [Гость]
- Сохраняй номера столов, все детали, тональность и смысл.
- Исправляй очевидные опечатки.
- Разделяй отзывы, которые слиплись в один блок.
- Никаких объяснений, никаких комментариев, никакого markdown.
...
Чистый стандартизированный текст:"""
