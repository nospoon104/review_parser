...
def build_prompt(raw_text: str) -> str:
    return f"""Ты — чистильщик отзывов для автоматического парсера ресторана.

Твоя задача — выдать максимально чистый текст, который идеально будет обработан существующим парсером.

Правила (обязательно):
- Каждый отзыв начинай строго с "Стол X: " в начале строки.
- Убирай все слова "гость", "Гость:", "гость:" — они только мешают.
- Убирай все префиксы "[Дата неизвестна]", "[Гость]" и подобные.
- Реплики менеджеров (">Алексей:", "Максим, добавь это...") оставляй как отдельные строки.
...
Выдай ТОЛЬКО очищенный текст, без каких-либо пояснений:"""
